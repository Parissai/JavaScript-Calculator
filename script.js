...
function dropLastChar() {           ///issue backspace makes display split  4π√log4  <=   4*Math.PI*Math.sqrt(*Math.log(
    ...
    if (cached.length === 0) { 
        ...
        display.innerHTML = cached.join('');//(display.innerHTML).slice(0, -1);  //TODO change for log
    }
}
